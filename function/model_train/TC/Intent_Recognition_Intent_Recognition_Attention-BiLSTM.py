...
SAVE_PATH = '../../saved_models/Intent_Recognition_Attention-BiLSTM'
EXCEL_PATH = '../../assess/Intent_Recognition_Attention-BiLSTM.xlsx'
MAX_LENGTH = 128
BATCH_SIZE = 32
EMBEDDING_DIM = 256
HIDDEN_DIM = 256
...
class ImprovedBiLSTM(nn.Module):
    ...
    def forward(self, text, lengths):
        ...
        # 解包序列
        out, _ = pad_packed_sequence(packed_out, batch_first=True)  # [batch_size, seq_len, hid_dim*2]
        
        # 注意力计算
        attn_weights = torch.softmax(self.attention(out).squeeze(2), dim=1)  # [batch_size, seq_len]
        context = torch.bmm(attn_weights.unsqueeze(1), out).squeeze(1)  # [batch_size, hid_dim*2]
...
def train_epoch(model, dataloader, optimizer, criterion, device):
    ...
    for batch in dataloader:
        ...
        lengths = batch['length'].to('cpu')  # 长度需要放在CPU
