...
# 封装的核心函数
# ====================
def start(data_path, save_path, device, model_path, max_len=128, epochs=20):
    """加载数据、训练模型并保存"""
...
if __name__ == "__main__":
    ...
    save_path = "saved_model_bert"  # 模型保存路径
    model_path = "bert-base-chinese"  # 模型路径
    max_len = 128
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")

    # 启动训练
    start(data_path, save_path, device, model_path, max_len)
